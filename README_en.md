...
- **Image Resize** — On-the-fly image resizing via query parameters with safety limits
- **Auto Image Resize** — Automatically resize large images on upload (S3 and WebDAV)
- **WebDAV Support** — Full WebDAV server for file management with S3 backend
...
webdav:
  enabled: false              # Enable WebDAV server
  listen: ":9002"             # WebDAV port (separate from S3/Admin)
  prefix: "/"                 # WebDAV URL prefix

image_resize:
  max_width: 4096             # Max output width (px), prevents memory explosion
...
Add query parameters to any image GET request to get a resized version on the fly:
...
| `m` | Resize mode: `fit` (default), `fill`, `crop` | `fit` |

**Resize modes:**
- `fit` — Scale down to fit within w×h, preserving aspect ratio (no upscaling)
- `fill` — Scale to fill w×h, center-crop overflow
- `crop` — Crop from center to exact w×h

**Examples:**
```bash
# Fit into 800x600 box (preserves aspect ratio)
...
# Crop to exact 200x200 thumbnail
...
**Safety limits** (configurable via `image_resize`):
- Output dimensions capped at `max_width` × `max_height` (default 4096×4096)
- Upscaling capped at `max_upscale` × original size (default 2×)
- Concurrent resizes capped at `max_concurrent` (default 4)

### Auto Resize on Upload

When enabled, large images uploaded via S3 PutObject or WebDAV are automatically resized asynchronously in the background:
...
Cloodsy S3 includes a built-in WebDAV server that maps directly to S3 buckets. This allows file management using any WebDAV client (Windows Explorer, macOS Finder, rclone, Cyberduck, etc.).
...
- Authentication: Use any S3 bucket credential (access key = username, secret key = password)
- Each bucket appears as a folder

**Supported operations:**
- PROPFIND — List files and folders
- GET — Download files
- PUT — Upload files (triggers auto-resize if enabled)
- MKCOL — Create folders
- DELETE — Delete files and folders
- COPY / MOVE — Copy and move files

**Notes:**
- WebDAV uploads go through the same S3 storage backend
- Auto-resize on upload works for both S3 API and WebDAV uploads
- File content types are auto-detected from extensions (`.jpg` → `image/jpeg`, etc.)
