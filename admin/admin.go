...
type Handler struct {
	...
	Logger        *slog.Logger
	Version       string
	WebDAVManager *wd.ServerManager
...
func (h *Handler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	// GET /admin/webdav — get WebDAV status
	case path == "/webdav" && r.Method == http.MethodGet:
		h.handleWebDAVStatus(w, r)

	// PUT /admin/webdav — toggle WebDAV on/off
	case path == "/webdav" && r.Method == http.MethodPut:
		h.handleWebDAVToggle(w, r)
...
func (h *Handler) handleWebDAVStatus(w http.ResponseWriter, r *http.Request) {
	if h.WebDAVManager == nil {
		writeJSON(w, http.StatusOK, map[string]interface{}{
			"enabled": false,
	...
	writeJSON(w, http.StatusOK, h.WebDAVManager.Status())
}

func (h *Handler) handleWebDAVToggle(w http.ResponseWriter, r *http.Request) {
	if h.WebDAVManager == nil {
		writeJSON(w, http.StatusBadRequest, map[string]string{"error": "webdav not configured"})
		return
	}
	...
	if req.Enabled {
		if h.WebDAVManager.IsRunning() {
			writeJSON(w, http.StatusOK, map[string]interface{}{
				"enabled": true,
				"message": "webdav already running",
			})
			return
		}
		if err := h.WebDAVManager.Start(); err != nil {
			h.Logger.Error("failed to start webdav", "error", err)
			writeJSON(w, http.StatusInternalServerError, map[string]string{"error": err.Error()})
			return
		}
		h.Logger.Info("webdav server started via admin API")
		writeJSON(w, http.StatusOK, map[string]interface{}{
			"enabled": true,
		...
		if !h.WebDAVManager.IsRunning() {
			writeJSON(w, http.StatusOK, map[string]interface{}{
				"enabled": false,
				"message": "webdav already stopped",
			})
			return
		}
		if err := h.WebDAVManager.Stop(); err != nil {
			h.Logger.Error("failed to stop webdav", "error", err)
			writeJSON(w, http.StatusInternalServerError, map[string]string{"error": err.Error()})
			return
		}
		h.Logger.Info("webdav server stopped via admin API")
		writeJSON(w, http.StatusOK, map[string]interface{}{
			"enabled": false,
			"message": "webdav server stopped",
		})
	}
}
